...
<g:set var="entityName"
	   value="${message(code: 'enrollment.label', default: 'Enrollment')}" />
<title>
...
<body>
	...
	<div class="body">
			...
			<g:if test="${enrollmentInstance.course}">
				Add students to
					<g:link class="uline"
							controller="course"
							action="show"
					        id="${enrollmentInstance.course.id}">
						${enrollmentInstance.course.name}
					</g:link>
			</g:if>
			<g:else>
				<g:message code="default.create.label" args="[entityName]" />
						...
						<tr class="prop">
							<g:if test="${enrollmentInstance.course}">
								<td valign="top" class="name">
									<label for="course">
										${enrollmentInstance.course.enrollments ?
                                            "Students already enrolled:" :
                                            "No students are enrolled yet" }
									</label>
								</td>
								<td valign="top" class="value">
									<g:each in="${enrollmentInstance.course.enrollments.sort({it.person.name})}"
											var="e">
										<span id="${e.person.id}">
											<br/>
							...
							<g:else>
								...
								<td valign="top"
									class="value ${hasErrors(bean: enrollmentInstance, field: 'course', 'errors')}">
									<g:select name="course.id"
											  from="${survey.Course.list()}"
											  optionKey="id"
											  value="${enrollmentInstance?.course?.id}" />
								</td>
							</g:else>
						</tr>
						<tr class="prop">
							...
							<td valign="top"
								class="value ${hasErrors(bean: enrollmentInstance, field: 'person', 'errors')}">
								<g:if test="${hasAvailableStudents}">
									<g:select id="availableStudents"
											  name="person.id"
											  from="${availableStudents}"
											  optionKey="id"
											  value="${enrollmentInstance?.person?.id}" />
								</g:if>
								<g:else>
		...
		<g:form name="addForm" action="save">
			<h2>
				Add new individual to system and course:
			</h2>
			<div class="dialog">
				<table>
					<tbody>
						<g:hiddenField name="course.id"
							           value="${courseInstance ? courseInstance.id : params.course.id}" />
						<tr class="prop">
							<td valign="top" class="name">
								<label for="name">
							...
							<td valign="top"
								...
								<g:textField class="required" name="name" value="${personInstance?.name}" />
							</td>
						</tr>
						...
						<tr class="prop">
							...
							<td valign="top"
								...
								<label for="email" class="error" style="display:none;">
